...
{
    SwapChain::SwapChain()
        : device_(VK_NULL_HANDLE), swap_chain_(VK_NULL_HANDLE), image_format_(VK_FORMAT_UNDEFINED), extent_({0, 0})
    {
    }
    ...
    {
        return vkAcquireNextImageKHR(device_, swap_chain_, std::numeric_limits<uint64_t>::max(),
                                     imageAvailableSemaphore, fence, imageIndex);
    }
